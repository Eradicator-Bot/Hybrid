				...
				if(istype(H) && can_burn)
					...
					if(temp_to_burn_with > H.base_body_temp + (H.temp_tolerance * 4) && !H.is_heat_resistant())
						if (chem_helmet_check(H, "hot"))
							...
							H.TakeDamage("head", 0, (temp_to_burn_with / 25) * dmg_multiplier, 0, DAMAGE_BURN) // lol this caused brute damage
							H.emote("scream")
					...
					if(iscarbon(A))
						...
						if(C.bioHolder)
							if(temp_to_burn_with > C.base_body_temp + (C.temp_tolerance * 4) && !C.is_heat_resistant())
								...
								C.TakeDamage("chest", 0, (temp_to_burn_with / 25) * dmg_multiplier, 0, DAMAGE_BURN)
