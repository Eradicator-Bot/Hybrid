...
/mob/living/carbon/human/emote(var/act, var/voluntary = 0, var/emoteTarget = null) //mbc : if voluntary is 2, it's a hotkeyed emote and that means that we can skip the findtext check. I am sorry, cleanup later
	...
	//april fools end
			...
			if ("miranda")
				if (src.emote_check(voluntary, 50))
					if (src.mind && (src.mind.assigned_role in list("Captain", "Head of Personnel", "Head of Security", "Security Officer",, "Officer Paramedic", "Security Assistant", "Detective", "Vice Officer", "Regional Director", "Inspector")))
						src.recite_miranda()
