	...
	New()
		...
		if (rand(1, 1000) == 69 && ticker?.mode) //heh
			...
			if (istype(ticker.mode, /datum/game_mode/nuclear))
				...
				name = "Drone [pick(list("Princess", "Lord", "King", "Queen", "Duke", "Baron"))] [pick(list("Bubblegum", "Wiffleypop", "Shnookems", "Cutesypie", "Rolypoly"))]"
	...
	emote(var/act, var/voluntary = 1)
		...
		switch(lowertext(act))
			...
			if ("list")
				...
				src.show_text("clap, flap, aflap, twitch, twitch_s, scream, birdwell, flip, custom, customv, customh")
